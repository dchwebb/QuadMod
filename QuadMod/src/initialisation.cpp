...
void InitSAI()
{
	...
	GPIOD->AFR[0] |= (10 << GPIO_AFRH_AFSEL11_Pos);
	...
	SAI1_Block_B->IMR |= SAI_xIMR_FREQIE;				// Interrupt on FIFO empty (for transmitter means data ready)

	NVIC_SetPriority(SAI1_IRQn, 1);						// Lower is higher priority
	NVIC_EnableIRQ(SAI1_IRQn);

	//SAI1_Block_A->CR2 |= SAI_xCR2_MUTE;				// Start SAI in muted mode for testing
	SAI1_Block_A->DR = 0;								// Load the FIFO with the first two left/right values
	SAI1_Block_A->DR = 0;

	// Initialise SAI2 as slave receiver
