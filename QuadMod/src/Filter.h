...
// n channel LP or HP filter with dual sets of coefficients to allow clean recalculation and switching
template<uint32_t poles = 2, uint32_t channels = 4>
struct Filter {
	static constexpr uint32_t sections = (poles + 1) / 2;
public:
...
private:
	...
	void InitIIRFilter(const float tone)	// tone is a 0-4095 number representing cutoff generally from ADC input
	{
		float cutoff;
		...
		if (passType == filterPass::HighPass || passType == filterPass::BandPass) {		// Want a sweep from 0.03 to 0.99 with most travel at low end
			cutoff = pow((tone / 5000.0), 3.0) + HPMin;
		} else {																		// Want a sweep from 0.001 to 0.2-0.3
			cutoff = std::min(0.03f + std::pow(tone / 4095.0f, 2.0f), LPMax);
		}
		iirFilter[inactiveFilter].CalcCoeff(cutoff);
