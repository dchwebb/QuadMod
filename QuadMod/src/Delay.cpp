...
void Delay::GetSamples(Samples& samples)
{
	...
	if (clockValid) {
		if (abs(hysteresisPotVal - adc.delayTimePot) > tempoHysteresis) {
			...
			float delayMult = tempoMult[((float)adc.delayTimePot / 4096.0f) * tempoMult.size()];		// get tempo multiplier from lookup
	...
	// LED - start at full brightness on clock tick and fade out
	...
	if (ledBrightness > 0) {
		...
		if (clockValid) {
			...
			unclockedDelayLED = ledBrightness >> 2;			// Add a little red to the green to make orange
		} else {
